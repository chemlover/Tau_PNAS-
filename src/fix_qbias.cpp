  ...
  Fix(lmp, narg, arg)
{
   ...
   //ffile=fopen("q_dump","w");
	...
	qdiff_flag = 0;
	epsilon = 1.0;
	strcpy(rnative_file, "rnative.dat");
	...
	while (!in.eof()) {
		...
		if (strcmp(varsection, "[Epsilon]")==0) {
			...
			in >> cutoff;
                        in >> rnative_file;
		}  else if (strcmp(varsection, "[QDiff]")==0) {
			qdiff_flag = 1;
			print_log("QDiff flag on\n");
			in >> k_qdiffbias;
			in >> q0;
			in >> l;
			in >> sigma_exp;
			in >> cutoff;
                        in >> rnative_file;
                        in >> rnative2_file;
		}	else if (strcmp(varsection, "[QinterfaceBias]")==0) {
			qinterfacebias_flag = 1;
			print_log("QinterfaceBias flag on\n");
			in >> k_qbias;
			in >> q0;
			in >> l;
			in >> sigma_exp;
	...
	if (qdiff_flag) {
	  ifstream in_rnative2(rnative2_file);
	  if (!in_rnative2) error->all(FLERR,"Qdiff simulation! The second rnative.dat file doesn't exist!");
	  for (i=0;i<n;++i){
	    for (j=0;j<n;++j) {
	      in_rnative2 >> rN2[i][j];
	    }
	...
	min_sep = 3;
	if (qobias_flag || qobias_exp_flag || qdiff_flag) min_sep=4;

	for (i=0;i<n;i++) {
	  sigma_sq[i] = Sigma(i)*Sigma(i);
	}
	// compute the necessary normalization constants
	Q1 = compute_qdiffconstant(rN);
	Q2 = compute_qdiffconstant(rN2);
...
{
  if (qbias_exp_flag || qobias_exp_flag || qdiff_flag || qinterfacebias_flag) {
...
//	a = 2.0/((n-2)*(n-3));
	for (i=0;i<n;++i) {
		for (j=i+min_sep;j<n;++j) {
			if ( (qobias_flag || qbias_exp_flag || qobias_exp_flag) && rN[i][j]>=cutoff ) continue;
...
void FixQBias::compute_qinterfacebias() 
{
	double qsum, a, dx[3], dr, dql1, dql;
	double force, force1;
	int i, j;
...
//	printf("%d %d %d %d\n",start1, end1, start2, end2);
	for (i=start1;i<end1;++i) {
		for (j=start2;j<end2;++j) {
			...
			dr = r[i][j] - rN[i][j];
//			printf( "%f %f\n", r[i][j],rN[i][j]);
			q[i][j] = exp(-dr*dr/(2*sigma_sq[214]));
//			printf( "%f %f  %f\n", r[i][j],rN[i][j],q[i][j]);
			qsum += q[i][j];
			a +=1.0;
		}
...
//	printf( "%f %f\n", a, qsum);
	...
	for (i=start1;i<end1;++i) {
		for (j=start2;j<end2;++j) {
			...
			force1 = force*q[i][j]*dr/r[i][j]/sigma_sq[214];
...
void FixQBias::compute_qdiffbias() 
{
  double qsum, a, dx[3], dr, dr2, dql1, dql;
  ...
  for (i=0;i<n;++i) {
    for (j=i+min_sep;j<n;++j) {
      if ( (qdiff_flag) && (rN[i][j]>=cutoff && rN2[i][j]>=cutoff ) ) continue;
      ...
      dr2 = r[i][j] - rN2[i][j];
      q[i][j] = exp(-dr*dr/(2*sigma_sq[j-i]));
      q2[i][j]= exp(-dr2*dr2/(2*sigma_sq[j-i]));
  ...
  energy[ET_QBIAS] += epsilon*k_qdiffbias*dql;

  force = epsilon*k_qdiffbias*dql1*l/a/(Q1-Q2);
  ...
  for (i=0;i<n;++i) {
    for (j=i+min_sep;j<n;++j) {
      ...
      force1 =  force*q[i][j] *dr /r[i][j]/sigma_sq[j-i];
      force2 = -force*q2[i][j]*dr2/r[i][j]/sigma_sq[j-i];
      force3 = force1 + force2 ;
      //printf("%f %f %f %f\n",r[i][j],sigma_sq[j-i],Q1,Q2);

      f[alpha_carbons[i]][0] -= -dx[0]*force3;
      f[alpha_carbons[i]][1] -= -dx[1]*force3;
      f[alpha_carbons[i]][2] -= -dx[2]*force3;

      f[alpha_carbons[j]][0] -= dx[0]*force3;
      f[alpha_carbons[j]][1] -= dx[1]*force3;
      f[alpha_carbons[j]][2] -= dx[2]*force3;
    }
  }
}

double FixQBias::compute_qdiffconstant(double **rStructure) 
{
  double qsum, a, dx[3], dr, dr2;
  int i, j;
  qsum = 0.0;
  a = 0.0;
  for (i=0;i<n;++i) {
    for (j=i+min_sep;j<n;++j) {
      ...
      r[i][j] = sqrt(dx[0]*dx[0]+dx[1]*dx[1]+dx[2]*dx[2]);
      dr  = rStructure[i][j] - rN[i][j];
      dr2 = rStructure[i][j] - rN2[i][j];
      //printf("%f \n",rN2[i][j]);
      q[i][j]  = exp(-dr*dr/(2*sigma_sq[j-i]));
      q2[i][j] = 1-exp(-dr2*dr2/(2*sigma_sq[j-i]));
      //printf("%f %f\n",sigma_sq[j-i],qsum/a);
      qsum += q[i][j]+q2[i][j];
      a +=1.0;
    }
...
void FixQBias::compute()
{
	...
	if (qbias_flag || qbias_exp_flag || qobias_flag || qobias_exp_flag)
		compute_qbias();
	if (qinterfacebias_flag)
		compute_qinterfacebias();
	if (qdiff_flag) {
	  compute_qdiffbias();
	}
	if ((qbias_flag || qbias_exp_flag || qobias_flag || qobias_exp_flag || qinterfacebias_flag) && Step>=sStep && Step<=eStep) {
