...
Fragment_Memory::Fragment_Memory(int p, int pf, int l, double w, char *fname, bool vec_fm_flag, bool CustomGoPairsFlag, char * CaCaGoPairsFile, char * CaCbGoPairsFile)
{
...
//  fprintf(dout, "%s\n", fname);
  ...
  // added by BIn
  customGoFlag = CustomGoPairsFlag;
  if (customGoFlag) {

	ifstream in_rnative(CaCaGoPairsFile);
        if (!in_rnative) { error = ERR_CUSTOM_GO; return; }
	for (i=0;i<len;++i) {
	    for (j=0;j<len;++j) {
		in_rnative >> rf[0][i][j];
            }
	...
	in_rnative.close();
        //
	in_rnative.open(CaCbGoPairsFile);
        if (!in_rnative) { error = ERR_CUSTOM_GO; return; }
	for (i=0;i<len;++i) {
	    for (j=0;j<len;++j) {
		in_rnative >> rf[1][i][j];
	...
	in_rnative.close();
        ...
        for (i=0;i<len;++i) {
          for (j=0;j<len;++j) {
            fprintf(dout, "%9.3f ", rf[0][i][j]);
          }
            fprintf(dout, "\n");
        }
        for (i=0;i<len;++i) {
          for (j=0;j<len;++j) {
            fprintf(dout, "%9.3f ", rf[1][i][j]);
          }
            fprintf(dout, "\n");
        }
        */
    ...
    for (i=0;i<len;++i) {
      for (j=0;j<len;++j) {
        ...
        // if i < j, set the value as alpha-alpha distance, 
        // if i > j, set as beta-beta distance
        // if i = j, set as 0.0
