...
void Pair3spn2::compute(int eflag, int vflag)
{
  ...
  // Get the temperature from the thermostat if performing parallel tempering
  int tmp,tstat_id, me;
  double t_target;
  if (tmp_name) {
  ...
  double qqr2e = force->qqr2e*0.36; // added by bin; Now I can use 1 for both protein and DNA in the data file.
...
void Pair3spn2::settings(int narg, char **arg)
{
  ...
  temp = force->numeric(arg[1]);
  //salt_conc = force->numeric(FLERR,arg[2]) / 1000.0; // Convert to [M]
  salt_conc = force->numeric(arg[2]) / 1000.0; // Convert to [M]
  ...
  ldby = sqrt(dielectric * _PV_ * _KB_ * temp * 1.0E27 / (2.0 * _NA_ * _EC_ * _EC_ * salt_conc));
  //printf("Dielectric=%lf\nDebye Length=%lf\n",dielectric, ldby);

  //cut_lj_global = force->numeric(FLERR,arg[3]);
  cut_lj_global = force->numeric(arg[3]);
  //cut_coul_global = ldby * 4.0; // Cutoff is 4 times the Debye length
  //cut_coul_global = force->numeric(FLERR,arg[4]); // This cutoff must be the same as that for coul/long
  cut_coul_global = force->numeric(arg[4]); // This cutoff must be the same as that for coul/long
