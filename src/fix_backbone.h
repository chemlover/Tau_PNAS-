...
Monte Carlo Protein Design contributed by Mingchen Chen

Amylometer contributed by Mingchen Chen

Tertiary Bias from Homologs (Native-Go) contributed by Mingchen Chen

Last Update: 06/01/2017 by Mingchen Chen
------------------------------------------------------------------------- */
...
public:
  ...
  double well_r_min_cut[5], well_r_max_cut[5]; //BinZhang
  ...
  double helix_well_r_min[5], helix_well_r_max[5], helix_well_r_min_cut[5], helix_well_r_max_cut[5];
  ...
  double **r_nativeCACB;
    ...
    // Native Bias parameters
  int native_bias_flag;
  int l;
  char rnative_file[100];
  double q0, sigma_exp, cutoff_bias;
...
//  TBV ****fm_table;
  ...
  static const double k_b = 0.001987;
   ...
   // Protein design 
  int protein_design_mode;
  int num_design_res;
  int num_monomer_res;
  char resfile_name[1000];
  int mm_resfile[10000];
  int protein_design_flag;
  int rand_seed;
  
  //Monte Carlo sampling sequence
  char native[1000];
  int monte_carlo_sampling_sequence_flag;
  char native_seqfile[100];
  double k_bias;
  double identity;
  
    //Monte Carlo sampling sequence 2D 
  char native2[1000];
  int monte_carlo_sampling_sequence_2d_flag;
  char native_seqfile2[100];
  double k_bias2;
  double identity2;
  ...
  /*
  // added by Bin for go pair exclusion
  // */
  bool read_custom_go_pairs_flag;
  char caca_go_pairs_file[200], cacb_go_pairs_file[200]; // not used currently
  ...
  double energy[21], energy_all[21];
  enum EnergyTerms{ET_TOTAL=0, ET_CHAIN, ET_SHAKE, ET_CHI, ET_RAMA, ET_VEXCLUDED, ET_DSSP, ET_PAP, 
		   ET_WATER, ET_BURIAL, ET_HELIX, ET_AMHGO, ET_FRAGMEM, ET_VFRAGMEM, ET_MEMB, ET_SSB, ET_DH, ET_Qbias, nEnergyTerms};
  
  double ctime[21], previous_time;
  enum ComputeTime{TIME_CHAIN=0, TIME_SHAKE, TIME_CHI, TIME_RAMA, TIME_VEXCLUDED, TIME_DSSP, TIME_PAP, 
		   TIME_WATER, TIME_BURIAL, TIME_HELIX, TIME_AMHGO, TIME_FRAGMEM, TIME_VFRAGMEM, TIME_MEMB, TIME_SSB, TIME_DH,TIME_Qbias, TIME_N};
 ...
 private:
  ...
  double compute_zscore();

  // protein design
  void compute_mcso_design();
  void compute_mcso_qbias();
  void compute_mcso_2Dqbias();
