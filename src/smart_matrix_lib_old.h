...
typedef struct WPV {
  ...
  double well_r_min[5], well_r_max[5], well_r_min_cut[5], well_r_max_cut[5], well_r_min_cut2[5], well_r_max_cut2[5];    //BinZhang
  WPV() {}
  WPV(double k, double ks, double t, int nw, int *wflag, double *wr_min, double *wr_max, double *wr_min_cut, double *wr_max_cut):   // BinZhang
  	  kappa(k), kappa_sigma(ks), treshold(t), n_wells(nw)
  {
  	for (int i=0; i<n_wells; ++i) {
  		...
  		well_r_min_cut[i] = wr_min_cut[i]; //BinZhang
  		well_r_max_cut[i] = wr_max_cut[i]; //BinZhang
  		well_r_min_cut2[i] = wr_min_cut[i] * wr_min_cut[i]; //BinZhang
  		well_r_max_cut2[i] = wr_max_cut[i] * wr_max_cut[i]; //BinZhang
  	}
  }
  WPV &operator = (const WPV &x) { 
  	...
  	for (int i=0; i<n_wells; ++i) {
  		...
  		well_r_min_cut[i] = x.well_r_min_cut[i]; //BinZhang
  		well_r_max_cut[i] = x.well_r_max_cut[i]; //BinZhang
  		well_r_min_cut2[i] = x.well_r_min_cut[i] * x.well_r_min_cut[i]; //BinZhang
  		well_r_max_cut2[i] = x.well_r_max_cut[i] * x.well_r_max_cut[i]; //BinZhang
  	}
...
cWell<T, U>::cWell(int nn, int mm, int ww, const WPV &p, int *indicator, U *lclass)
{
	...
	for (int i=0;i<n;++i) {
        ...
        ntot = nw*nm;
	v_theta = new T[ntot];
	v_prd_theta = new T[ntot];
	gTheta = new int[ntot];
        for (int i=0; i<ntot; ++i) gTheta[i]=-1;
        /*
	v_theta = new T**[nw];
...
inline T &cWell<T, U>::theta(int i, int j, int i_well)
{
	...
	if ( (ind && gTheta[ij]!=*ind) || (!ind && gTheta[ij]!=1) ) {
		compute_theta(i, j, i_well);
		if (ind) gTheta[ij] = gTheta[ji] = *ind;
		else gTheta[ij] = gTheta[ji] = 1;
	}

	//return v_theta[i_well][i][j];
	return v_theta[i_well*nm+i*m+j];
}
...
void cWell<T, U>::compute_theta(int i, int j, int i_well)
{
	...
	rij2 = pow(dx[0],2) + pow(dx[1],2) + pow(dx[2],2);
        ...
        if ((rij2<par.well_r_min_cut2[i_well]) || (rij2>par.well_r_max_cut2[i_well])) { //BinZhang
            //printf("outter here: %d, %12.6f, %12.6f, %12.6f\n", i_well, rij2, par.well_r_min_cut2[i_well], par.well_r_max_cut2[i_well]);
            v_theta[ij] = 0.0;
            v_theta[ji] = 0.0;
            v_prd_theta[ij] = 0.0;
            v_prd_theta[ji] = 0.0;
        } else {
            //printf("inner here: %d, %12.6f, %12.6f, %12.6f\n", i_well, rij2, par.well_r_min_cut2[i_well], par.well_r_max_cut2[i_well]);
            rij = sqrt(rij2);
            t_min = tanh( par.kappa*(rij - par.well_r_min[i_well]) );
            t_max = tanh( par.kappa*(par.well_r_max[i_well] - rij) );
	    //t_min = tanh_approx( par.kappa*(rij - par.well_r_min[i_well]) );
	    //t_max = tanh_approx( par.kappa*(par.well_r_max[i_well] - rij) );
            v_theta[ij] = 0.25*(1.0 + t_min)*(1.0 + t_max);
            v_theta[ji] = v_theta[ij];
            v_prd_theta[ij] = par.kappa*v_theta[ij]*(t_max - t_min)/rij;
            v_prd_theta[ji] = v_prd_theta[ij];
        }
...
inline double tanh_approx(double r)
{
  double r2, ar, br;
  r2 = r*r;
  ar = (((r2+378)*r2+17325)*r2+135135)*r;
  br = ((28*r2+3150)*r2+62370)*r2 + 135135;
  return ar/br;
  //return r/(1 + r2/(3 + r2/(5 + r2/(7 + r2/(9 + r2/(11+r2/(13 )))))));
}
