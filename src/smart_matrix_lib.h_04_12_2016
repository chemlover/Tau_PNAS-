...
inline T &cWell<T, U>::theta(int i, int j, int i_well)
{
	if ( (ind && gTheta[i_well][i][j]!=*ind) || (!ind && gTheta[i_well][i][j]!=1) ) {
		...
		if (ind) gTheta[i_well][i][j] = *ind;
		else gTheta[i_well][i][j] = 1;
	}
...
void cWell<T, U>::compute_theta(int i, int j, int i_well)
{
	...
	rij2 = pow(dx[0],2) + pow(dx[1],2) + pow(dx[2],2);
        ...
        if ((rij2<par.well_r_min_cut2[i_well]) || (rij2>par.well_r_max_cut2[i_well])) { //BinZhang
            v_theta[i_well][i][j] = 0.0;
            //v_theta[i_well][j][i] = 0.0;
            v_prd_theta[i_well][i][j] = 0.0;
            //v_prd_theta[i_well][j][i] = 0.0;
        } else {
	    rij = sqrt(rij2);
